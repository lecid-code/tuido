...
class TaskCLI:
    ...
    def run(self, args=None):
        ...
        if parsed_args.verbose or parsed_args.file != DEFAULT_TASK_FILE:
            self.console.print(Panel(f":file_folder: Using task file: [cyan]{parsed_args.file}[/cyan]",
                          border_style="cyan"
            ))
            self.console.print()
    ...
    def _handle_add(self, task_manager: TaskManager, description: str):
        ...
        self.console.print(f"[bold green]✓[/bold green] Added task {new_task.id}: '{new_task.description}'")
    ...
    def _handle_list(self, task_manager: TaskManager):
        ...
        if completed_tasks:
            self.console.print(f"[green]COMPLETED TASKS ({len(completed_tasks)})[/green]")
        ...
        if not tasks:
            self.console.print("[dim]No tasks found. Use 'add' command to create a new task.[/dim]")

    def _handle_do(self, task_manager: TaskManager, task_id: int):
        if task_manager.set_task_complete(task_id):
            self.console.print(f"[bold green]✓[/bold green] Task {task_id} marked as complete.")
        else:
    ...
    def _handle_undo(self, task_manager: TaskManager, task_id: int):
        if task_manager.set_task_pending(task_id):
            self.console.print(f"[bold green]✓[/bold green] Task {task_id} marked pending.")
        else:
    ...
    def _handle_delete(self, task_manager: TaskManager, task_id: int):
        ...
        if task is not None:
            self.console.print(f"[bold green]✓[/bold green] Deleted task {task.id}: '{task.description}'.")
        else:
